---
title: "From Search to Memory: 35 Million Words of AI Conversations"
site: arjunsriva
...
Matches:  6 distinct implementation surfaces across 3 tools
          ...
          - Fashion-MNIST benchmark run (Local repo log)
```
...
Relationships:     [pairs_with -> C234 (LeJEPA Pure-Python Reproduction)]
                   [implements_method -> JEPA Architecture]
```
...
This same evidentiary discipline applies across systems: [**OMA**](https://www.novusopus.org/oma/) has git commits, working code, and a Cloud Run deployment (*built prototype*), whereas early drafts of [**`pg_llm`**](https://www.novusopus.org/research/quantitative-qualitativeness/) contained SQL design specs and landing pages with a verified production C extension (*designed architecture*).
...
Corpus Quarantine Total:  965 shorter or duplicate files held from promotion
```
...
Projection B:  "What is the public editorial disposition of each project?"
               -> Mining Plan: 249 projects fully accounted for (62 shorts, 24 scheduled
                  flagships, 7 existing public assets, 0 unassigned).
```

Neither projection replaces the underlying evidence base; each compiles a temporary view shaped for the reader at hand.
...
I built [**OMA**](https://www.novusopus.org/oma/) in November 2025 to solve this, applying John Maeda’s *The Laws of Simplicity* and SLIP principles to Clio’s clustering. OMA rejected the assumption that personal archives should have one permanent taxonomy. Instead, it generates transient taxonomies on the fly from user intent:
...
In February 2025, inspired by Benn Stancil’s `AVG(text)` concept, I designed **`pg_llm`** (documented as [**Semantic SQL**](https://www.novusopus.org/research/quantitative-qualitativeness/)) to bring qualitative text analysis directly into SQL without external infrastructure overhead:
...
1. **Direct first-person records:** Authored user messages establishing what was intended, remembered, or proposed at an exact timestamp.
2. **Inspectable artifacts:** Commit logs, repositories, deployed URLs, and verified build outputs establishing that an artifact existed in that state.
3. **Derived repository views:** Assistant syntheses, project rosters, and generated summaries (useful for discovery, but requiring validation against raw artifacts).
4. **Assistant suggestions:** Conversational proposals establishing what an LLM suggested, not what was accepted or implemented.
